    ...
    public function onMessage($connection, $buffer)
    {
        ...
        switch($cmd)
        {
            case 'get':
                if(!isset($this->_dataArray[$key]))
                {
                    if(empty($data['wait'])){
                        ...
                        $this->_watch_key[$key] = $connection;
                        \Workerman\Timer::add($data['wait'],function($data){
                            if(isset($this->_watch_key[$data['key']])){
                                ($this->_watch_key[$data['key']])->send('N;');
                            }

                            unset($this->_watch_key[$data['key']]);
                            \Workerman\Timer::del($data['wait']);
                        },[$data]);
                        return;
                    }
            ...
            case 'set':
                ...
                var_dump(__FILE__.' line:'.__LINE__,$key);
                if(isset($this->_watch_key[$key])){

                    $c = $this->_watch_key[$key];
                    $c->send(serialize($this->_dataArray[$key]));
                }
                $connection->send('b:1;');
                break;
            case 'add':
